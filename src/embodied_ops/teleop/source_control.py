...
def request_alignment(endpoint: str, payload: bytes, *, timeout_ms: int = 2000) -> dict[str, Any]:
    """One socket per call: SDK service workers never share a ZMQ socket."""
    try:
        if len(payload) > 8192:
            raise ValueError("Alignment request too large")
        request = json.loads(payload)
        if not isinstance(request, dict):
            raise ValueError("Expected an alignment request object")
